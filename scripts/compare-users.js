...
(async () => {
    try {
        ...
        // Find common sailings
        user1SailingMap.forEach((user1SailingList, key) => {
            if (user2SailingMap.has(key)) {
                const user2SailingList = user2SailingMap.get(key);
                // Take the first sailing from each user for the match
                const user1Sailing = user1SailingList[0];
                const user2Sailing = user2SailingList[0];
                matchingSailings.push({
                    shipName: user1Sailing.shipName,
                    sailDate: user1Sailing.sailDate,
                    departurePort: user1Sailing.departurePort || user2Sailing.departurePort,
                    itinerary: user1Sailing.itinerary || user2Sailing.itinerary,
                    nights: user1Sailing.nights || user2Sailing.nights,
                    user1Offers: [...new Set(user1SailingList.map(s => s.offerCode))],
                    user2Offers: [...new Set(user2SailingList.map(s => s.offerCode))],
                    user1OfferNames: [...new Set(user1SailingList.map(s => s.offerName))],
                    user2OfferNames: [...new Set(user2SailingList.map(s => s.offerName))]
                });
            } else {
                // USER1 only
                user1SailingList.forEach(sailing => {
                    user1OnlySailings.push({
                        ...
                        offerName: sailing.offerName
                    });
                });
            }
        });

        // Find USER2 only sailings
        user2SailingMap.forEach((user2SailingList, key) => {
            if (!user1SailingMap.has(key)) {
                user2SailingList.forEach(sailing => {
                    user2OnlySailings.push({
                        shipName: sailing.shipName,
                        sailDate: sailing.sailDate,
                        departurePort: sailing.departurePort,
                        itinerary: sailing.itinerary,
